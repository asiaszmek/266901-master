...
PARAMETER {
	gbar = 0.001(cm/s)		: vh = 100 mV, 120 ms pulse to 0 mv
	...
	htau0 = 20      (ms)
        ...
        qfact = 2					: both m & h recorded at 22 C
}					
...
BREAKPOINT {
    ...
    ica  = ghk(v,cai,cao) * gbar * m * m * m * h	: Wang 1991
...
PROCEDURE rates( v (mV) ) {
		htau = htau0 + htau1*exp((-(v-thvhalf)/thvslope))
